...
class Export
{
     ...
     *  @return Export    $this
     */
    public function writeFullTable($data, $labels = array(), $options = array(), $columnsSise=array())
    {
        // Default coordinate of the table.
		$this->cursor['x'] = 0;
        $this->cursor['y'] = 1;

		$countColumns = count($labels);

        for($i = 0; $i < $countColumns; $i++)
        {
            $this->currentSheet->getColumnDimensionByColumn($i, $this->cursor['y'])->setWidth($columnsSise[$i]);        
        }
     ...
     *  @return Export    $this
     */
    public function writeTable($data, $size, $options = array(), $columnsSise=array())
    {
        // Default coordinate of the table.
        $this->cursor['x'] = 0;
        $this->cursor['y'] = 1;
   ...
   public function strongColumn($column, $countRows)
   {
      for($i = 2; $i <= $countRows + 1; $i++)
      {
      	 $this->currentSheet->getStyleByColumnAndRow($column, $i)
            ->getFont()
