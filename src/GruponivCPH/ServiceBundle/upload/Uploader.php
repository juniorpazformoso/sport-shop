...
namespace GruponivCPH\ServiceBundle\upload;
...
class Uploader {
     ...
     * @return array
     */
    public function upload($field, $name, $options = null){
        $this->data = $this->cache_data;
        return $this->initialize($field, $name, $options);
    }
     ...
     * @return array
     */
    private function initialize($field, $name, $options){
	...
	private function prepareFiles($name){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                $metas['name'] = $name;//$this->generateFileName($this->options['title'], array('name'=>$metas['old_name'], 'size'=>$metas['size'])) . (!empty($metas['extension']) ? "." . $metas['extension'] : "");
